...
class AutonomousEvolutionRunner:
    ...
    def run(self) -> dict[str, Any]:
        ...
        try:
            ...
            return self._block_integrity(
                ...
                round_root=(
                    self.output_root
                    / "rounds"
                    / f"round-{state.next_round_index:04d}"
                ),
                phase="best-harness-projection",
                error=error,
            )
        if state.status is not GoalRunStatus.ACTIVE:
            return self._terminal_result(state)
        prior_claims = replay.prior_claims
        historical_claims = list(replay.historical_claims)
        prior_campaign_feedback = replay.prior_campaign_feedback
        ...
        best_round_payload = replay.best_round_payload
        selection_counts = dict(replay.task_selection_counts)
        failure_counts = dict(replay.failure_signature_counts)
        ...
        for current_round in range(
            ...
            selection_context = TaskSelectionContext(
                historical_claims=tuple(historical_claims),
                current_best_revision_id=state.best_candidate_revision_id,
                current_best_supported_task_ids=(
                    _claim_task_ids(best_round_payload)
                    if best_round_payload is not None
                    else ()
                ),
                failure_signature_counts=failure_counts,
                goal_gap=max(
                    0,
                    self.config.goal.target_native_gains - len(gained_tasks),
                ),
                task_selection_counts=selection_counts,
                repeat_hard_cap=_repeat_hard_cap(
            ...
            failure_package = {
                ...
                "selected_tasks": [teacher_task(task) for task in selection.tasks],
                "baseline": baseline_payload,
                "prior_claims": [dict(row) for row in prior_claims],
                "prior_campaign_feedback": dict(prior_campaign_feedback),
            ...
            try:
                ...
                return self._block_integrity(
                    ...
                    proposer=proposer,
                    round_root=round_root,
                    phase="campaign-feedback-lineage",
                    error=error,
                )
                ...
                canonical_json(
                    {
                        ...
                        "paired_model_native_failures": (
                            campaign_failure_facts(campaign_feedback)
                        ),
                    }
                ).encode("utf-8")
            ).hexdigest()
            ...
            if accepted:
                ...
                verify_best_harness(
                    best_path,
                    expected_model_identity_sha256=identity_sha,
                    accepted_round=round_payload,
                )
            elif best_round_payload is not None:
                verify_best_harness(
                    best_path,
                    expected_model_identity_sha256=identity_sha,
                    accepted_round=best_round_payload,
                )
            else:
                verify_best_harness(
                    best_path,
            ...
            historical_claims.extend(claim_rows)
            prior_campaign_feedback = campaign_feedback
            for task_id in selection.selected_task_ids:
                selection_counts[task_id] = selection_counts.get(task_id, 0) + 1
            _accumulate_failure_counts(failure_counts, campaign_feedback)
            state = GoalState(
                schema_version=1,
    ...
    def _rebuild_completed_rounds(self, state: GoalState) -> _ReplayState:
        ...
        prior_campaign_feedback: Mapping[str, Any] = {}
        rejected: list[str] = []
        ...
        for sequence, row in enumerate(rows):
            ...
            if selection_payload.get("selection_id") != payload.get("selection_id"):
                raise AutonomousEvolutionError("round task selection identity drift")
            selected_task_ids = selection_payload.get("selected_task_ids")
            if not isinstance(selected_task_ids, list) or any(
                not isinstance(task_id, str) or not task_id
                for task_id in selected_task_ids
            ):
                raise AutonomousEvolutionError("round task selection is invalid")
            for task_id in selected_task_ids:
            ...
            if not isinstance(loaded_campaign_feedback, Mapping):
                raise AutonomousEvolutionError(
                    "round campaign feedback projection is invalid"
                )
            prior_campaign_feedback = dict(loaded_campaign_feedback)
    ...
    def _verify_best_projection(
        ...
        *,
        ...
        if replay.best_round_payload is None:
            if replay.state.best_candidate_revision_id is not None:
                raise ContractViolation("state names a best candidate without authority")
            compiled = verify_best_harness(
                best_path,
                expected_model_identity_sha256=model_identity_sha256,
            )
            if compiled is not None:
                raise ContractViolation(
                    "compiled BEST-HARNESS has no accepted round authority"
                )
            return
        if (
            replay.state.best_candidate_revision_id
            != replay.best_round_payload.get("candidate_revision_id")
            or replay.state.best_bundle_sha256
            != replay.best_round_payload.get("compiled_bundle_sha256")
        ):
            raise ContractViolation("state and accepted best authority disagree")
        verify_best_harness(
            best_path,
            expected_model_identity_sha256=model_identity_sha256,
            accepted_round=replay.best_round_payload,
        )
...
class _ReplayState:
    ...
    best_round_payload: Mapping[str, Any] | None
...
def _selection_payload(selection: TaskSelection) -> dict[str, Any]:
    return {
        ...
        "selection_context_sha256": selection.selection_context_sha256,
        "selection_context": selection.selection_context_payload(),
...
def _repeat_hard_cap(
    *, max_rounds: int, tasks_per_campaign: int, pool_size: int
) -> int:
    return max(1, (max_rounds * tasks_per_campaign + pool_size - 1) // pool_size)


def _claim_task_ids(payload: Mapping[str, Any]) -> tuple[str, ...]:
    claims = payload.get("claims")
    if not isinstance(claims, list):
        raise AutonomousEvolutionError("accepted best claims are invalid")
    task_ids: list[str] = []
    for claim in claims:
        if not isinstance(claim, Mapping):
            raise AutonomousEvolutionError("accepted best claim is invalid")
        task_id = claim.get("task_id")
        if not isinstance(task_id, str) or not task_id:
            raise AutonomousEvolutionError("accepted best claim task is invalid")
        task_ids.append(task_id)
    return tuple(sorted(set(task_ids)))


def _accumulate_failure_counts(
    counts: dict[str, int], campaign_feedback: Mapping[str, Any]
) -> None:
    pairs = campaign_feedback.get("task_pairs", ())
    if not isinstance(pairs, (list, tuple)):
        raise AutonomousEvolutionError("campaign feedback task pairs are invalid")
    for pair in pairs:
        if not isinstance(pair, Mapping):
            raise AutonomousEvolutionError("campaign feedback task pair is invalid")
        task_id = pair.get("task_id")
        claim = pair.get("claim")
        if not isinstance(task_id, str) or not task_id or not isinstance(claim, Mapping):
            raise AutonomousEvolutionError("campaign feedback failure identity is invalid")
        if claim.get("classification") != "gain":
