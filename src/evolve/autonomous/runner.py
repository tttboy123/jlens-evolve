    ...
    def run(self) -> dict[str, Any]:
        ...
        for current_round in range(
            ...
            failure_package = {
                ...
                "selected_tasks": [_teacher_task(task) for task in selection.tasks],
            ...
            try:
                campaign_feedback = _campaign_feedback(
                    round_root=round_root,
                ...
                canonical_json(
                    {
                        ...
                        "paired_model_native_failures": (
                            _campaign_failure_facts(campaign_feedback)
                        ),
                    }
                ).encode("utf-8")
            ).hexdigest()
    ...
    def _rebuild_completed_rounds(self, state: GoalState) -> _ReplayState:
        ...
        for sequence, row in enumerate(rows):
            ...
            if loaded_campaign_feedback is None:
                loaded_campaign_feedback = _rebuild_campaign_feedback(
                    round_root=round_root,
                    payload=payload,
                )
...
def _teacher_task(task: Mapping[str, Any]) -> dict[str, Any]:
    """Remove local checkout locations from the remotely dispatched task row."""

    return {
        key: value
        for key, value in task.items()
        if key not in {"source_uri", "source_repository"}
    }
    ...
    *, round_root: Path, payload: Mapping[str, Any]
    ...
    if campaign_status == "screened_out":
        ...
        if not isinstance(selected_task_ids, list) or not all(
            ...
            raise AutonomousEvolutionError(
                "legacy round task selection is invalid"
            )
        claims = CampaignOutcomeVerifier().verify(
            round_root=round_root,
            result=campaign_result,
...
def _campaign_feedback(
    *,
    ...
    """Project Teacher-safe paired failures from verified immutable receipts.

    The projection intentionally excludes artifact bodies (notably patches and
    prompts). Claim data only binds the pair to independently verified lineage;
    model and native failure facts always come from ReceiptStore.
    """
    ...
    campaign_id = _optional_text(
        campaign_result.get("campaign_id"), "campaign id"
    )
    if not claims:
    ...
    for plan_id, plan_receipts in receipts_by_plan.items():
        ...
        if (
            ...
            or external.payload.get("model_artifact_sha256")
        ...
        task_pairs.append(
            {
                ...
                "claim": {
                    ...
                    "counterfactual_pair_sha256": verified.counterfactual_pair_sha256,
...
def _prescreen_feedback(
    *,
    ...
    """Project a pre-native rejection from its immutable model/trace receipts."""

    if prescreen.status != "completed" or len(prescreen.model_receipt_ids) != 1:
        raise AutonomousEvolutionError(
            "screened-out candidate lacks one completed prescreen model receipt"
        )
    receipts = ReceiptStore(
        round_root / "prescreen/receipt-store"
    ).list_receipts()
    model = next(
        (
            receipt
            for receipt in receipts
            if receipt.receipt_id == prescreen.model_receipt_ids[0]
            and receipt.kind == "model"
        ),
        None,
    )
    if model is None:
        raise AutonomousEvolutionError(
            "screened-out candidate model receipt is missing"
        )
    ...
    if (
        ...
        or model.payload.get("candidate_revision_id")
        != prescreen.candidate_revision_id
        or model.payload.get("candidate_bundle_sha256")
        != prescreen.candidate_bundle_sha256
        or structural_valid != prescreen.structural_valid
    ...
    return {
        "candidate_id": candidate_id,
        "candidate_revision_id": prescreen.candidate_revision_id,
        "candidate_bundle_sha256": prescreen.candidate_bundle_sha256,
        "task_revision_id": _required_text(
        ...
        "patch_applicable": prescreen.patch_applicable,
        "failure_reason": failure_reason,
        "failure_reason_raw_sha256": failure_reason_raw_sha256,
    }


def _campaign_failure_facts(campaign_feedback: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(prescreen, Mapping):
        facts.append(
            {
                "prescreen": {
                    "candidate_revision_id": prescreen.get(
                        "candidate_revision_id"
                    ),
                    "candidate_bundle_sha256": prescreen.get(
                        "candidate_bundle_sha256"
                    ),
                    "task_revision_id": prescreen.get("task_revision_id"),
                    "patch_sha256": prescreen.get("patch_sha256"),
                    "structural_valid": prescreen.get("structural_valid"),
                    "patch_applicable": prescreen.get("patch_applicable"),
                    "failure_reason": prescreen.get("failure_reason"),
                }
            }
        )
    return facts
